...
const colorAddonTemplate = fs.readFileSync("templates/color.addon.template.css", "utf-8");
const colorAddonWhiteTemplate = fs.readFileSync("templates/color.addon.white.template.css", "utf-8");
...
const renderAddonColor = (color) =>
  color === "white" ? colorAddonWhiteTemplate : colorAddonTemplate.replace(/__COLOR__/g, color);

let colorOutput = `.har-input-wrapper {${colors.map(renderColor).join("\n")}}`;
let colorAddonOutput = `.har-input-wrapper {${colors.map(renderAddonColor).join("\n")}}`;
...
fs.writeFileSync("color.addon.css", colorAddonOutput);
fs.writeFileSync("radius.css", radiusOutput);
fs.writeFileSync("size.css", sizeOutput);

finalizeGeneratedCss(["color.css", "color.addon.css", "radius.css", "size.css"]);
